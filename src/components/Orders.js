...
function Orders({ items }) {
  ...
  // const loadOrders = () => {
  //   axios.get('/api/orders').then((result) => setOrders(result.data)).catch(console.error);
  // };
  ...
  return (
    <div className="orders-component">
      ...
      {orders.length === 0 ? <div>{currentUser.access === 'associate' ? 'No Orders' : 'Access Denied'}</div>
        : orders.map((order) => (
